...
class LedDisp
{
  ...
  public:
    ...
    // Constructor
    LedDisp(int,int,int,int,int,int,int);
...
//  int getDigitDelay();	// How many get-functions should I make?
  ...
  private:
    ...
    int _refreshDigit;		// What digit interrupt timer should update next time
    int _refreshPhase;		// What phase of the cycle it is to update, i.e. phase 1 (on), or phase 0 (off). Needed for duty cycling.
    int _refreshState;		// Values 0,1 corresponds to using off and on.
    long int _refreshCounter;		// Prescaler of 64 is used.
		long int _refreshCounterOnEnd;	// How far _timerCounter should count to provide a delay approximately equal to _digitOnDelay
    long int _refreshCounterOffEnd;	// How far _timerCounter should count to provide a delay approximately equal to _digitOffDelay
