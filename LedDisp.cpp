...
// Constructor
LedDisp::LedDisp(int A,int B,int C,int D,int E,int F,int G){
  ...
  // refresh data (default values when refresh is off)
  _refreshDigit=0;
  _refreshPhase=1;
  _refreshState=0;
  _refreshCounter=0;
  _refreshCounterOnEnd=0;
  _refreshCounterOffEnd=0;
...
void LedDisp::setRefresh(){
		 _refreshState = 1;
}

void LedDisp::setCommonAnode(){
  _digOn=HIGH;
  _digOff=LOW;
  _segOn=LOW;
  _segOff=HIGH;
...
*/
void LedDisp::setDigitPins(int numOfDigits, int *pDigitPins){
  _dig=pDigitPins;
...
void LedDisp::setDigitDelay(long int delay){
  _digitDelay=delay;
  updDelay();
}

void LedDisp::setDutyCycle(int dc){
  _dutyCycle=dc;
  updDelay();
}

void LedDisp::setActivePinState(int segActive, int digActive){
  _digOn = digActive;
  _digOff = !digActive;
  _segOn = segActive;
  _segOff = !segActive;
...
void LedDisp::setRefreshRate(int freq){
 ...
 * write(char* a)
 *
 * For the refresh mode, the value must simply be stored to a private variable, and printed by refreshDisp. For
 ...
 *
 * Further on, all functions should be using refresh if _refreshState!=-1. If not, the should multiplex through the
...
void LedDisp::writeClock(int ss){

  writeClock(ss/60,ss%60);

}

void LedDisp::writeClock(int ss, char c){

  writeClock(ss/60,ss%60,c);

}

void LedDisp::writeClock(int mm, int ss){
...
void LedDisp::writeClock(int mm, int ss, char c){

  if(_refreshState!=1){  // No refresh assigned. MUX once.
...
void LedDisp::write(int num,int point){
  write((long int)num, point);
}

void LedDisp::write(long int num,int point){
...
*/
	...
	_writeMode = 'p';	// Tell refreshAction that write(int,int) was used (fixed point).
	_writeInt = iaLimitInt(num);	// Tell refreshAction to write this number ...
        _writePoint = point;	// ... with this fixed point
  }

}

// Extracts digit number "digit" from "number" for use with ia - refreshtAction
char LedDisp::iaExtractDigit(long int number, int digit, int point){
...
long int LedDisp::iaLimitInt(long int number){
...
void LedDisp::write(int num){
  write((long int)num);
}

void LedDisp::write(long int num){
  ...
  if(_refreshState!=1){  // No refresh assigned. MUX once.
	...
	_writeMode = 'i';	// Tell refreshAction that write(int) is used.
	_writeInt = iaLimitInt(num);	// Tell refreshAction to write this int
  }

}

void LedDisp::write(char *str){
  ...
  if(_refreshState!=1){  // No refresh assigned. MUX once.
	...
	_writeMode = 's';	// Tell refreshAction that write(char*) is used.
	_writeStr = str;	// Tell refreshAction to write this string
  }


}

void LedDisp::write(String str){
  ...
  if(_refreshState!=1){  // No refresh assigned. MUX once.
	...
	_writeMode = 'o';	// Tell refreshAction that write(String) is used.
	_writeStrObj = str;	// Tell refreshAction to write this string
  }


}

void LedDisp::write(double num, int point){
...
void LedDisp::updDelay(){
  ...
  if(_refreshState!=-1){
    // Artefacts in duty cycle control appeared when these values changed while refresh happening (A kind of stepping in brightness appeared)
    _refreshCounterOnEnd=(_digitOnDelay/16)-1;
    _refreshCounterOffEnd=(_digitOffDelay/16)-1;
    if(_digitOnDelay==0) _refreshCounterOnEnd=0;
    if(_digitOffDelay==0) _refreshCounterOffEnd=0;
		_refreshCounter=0;
	}
}

void LedDisp::refreshDisp(){

  // Increment the library's counter
  _refreshCounter++;

  // Finished with on-part. Turn off digit, and switch to the off-phase (_refreshPhase=0)
  if((_refreshCounter>=_refreshCounterOnEnd)&&(_refreshPhase==1)){
    _refreshCounter=0;
    _refreshPhase=0;
  ...
  if((_refreshCounter>=_refreshCounterOffEnd)&&(_refreshPhase==0)){
    _refreshCounter=0;
    _refreshPhase=1;

    _refreshDigit++;

    if(_refreshDigit>=_numOfDigits){
      if(_symbDigPin!=-1 && _refreshDigit==_numOfDigits){  // Symbol pin in use. Let _refreshDigit=_numOfDigits be used for symbol mux.
      } else { // Finished muxing symbol digit, or symbol pin not in use
        _refreshDigit=0;
      }
    }

    if(_refreshDigit==_numOfDigits) changeDigit('s');
    else changeDigit(_refreshDigit);

    if(_refreshDigit!=_numOfDigits){

        if(_writeMode=='p'){	// Fixed point writing (or float)
          writeDigit(iaExtractDigit(_writeInt,_refreshDigit,_writePoint));
          if(_writePoint==_refreshDigit && _writePoint!=_numOfDigits-1) setDP();
        }

        if(_writeMode=='i'){	// Integer writing
          writeDigit(iaExtractDigit(_writeInt,_refreshDigit,_numOfDigits));
        }
        ...
        if(_writeMode==':'||_writeMode=='.'||_writeMode=='_'){
          ...
          if(_refreshDigit==_numOfDigits){	// Symbol digit
            setColon();
          } else {
            writeDigit(iaExtractDigit(_writeInt,_refreshDigit,_numOfDigits));
            if(_writeMode==':' && !symbColon) setColon();
            if((_writeMode=='.')&&(_refreshDigit==_numOfDigits-3)) setDP();  // Only set "." in the right place
          }
        ...
        if(_writeMode=='s'){
          ...
          while(_writeStr[i]!='\0' && j<_refreshDigit){
            if(_writeStr[i+1]=='.'){
              i++;
            }
            i++;
        ...
        if(_writeMode=='o'){
          ...
          int i=0; // which digit
          int j=0; // which digit have it counted to
          while(i<_writeStrObj.length() && j<_refreshDigit){
            if(_writeStrObj[i+1]=='.'){
              i++;
            }
            i++;
...
void LedDisp::changeDigit(int digit){
...
void LedDisp::changeDigit(char digit){
...
void LedDisp::setDPPin(int DPPin){
...
void LedDisp::setColonPin(int colonPin){
  _colonSegPin=colonPin;
  pinMode(_colonSegPin,OUTPUT);
  digitalWrite(_colonSegPin, _colonState);
}

void LedDisp::setSymbPins(int digPin, int segUCPin, int segLCPin, int segAPin){
...
void LedDisp::writeDigit(int digit){
...
void LedDisp::writeDigit(char digit){
...
void LedDisp::execDelay(int usec){
